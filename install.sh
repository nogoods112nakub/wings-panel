...
# get_daemon_token prints the effective daemon token: sourced .env > compose default.
get_daemon_token() {
    if [ -n "${DAEMON_TOKEN:-}" ]; then
        printf '%s' "$DAEMON_TOKEN"
        return
    fi
...
ensure_secrets() {
    ...
    if [ -f "$env_file" ]; then
        ...
        info "Using existing secrets from $env_file (not rotating them)"
        return 0
    fi

    if ! command -v openssl &>/dev/null; then
        fail "openssl is required to generate secure secrets. Install it (e.g. apt install openssl)."
    fi

    local jwt_secret daemon_token postgres_pw admin_pw
    ...
    if [ "$existing" = true ]; then
        info "Existing deployment detected — keeping database/admin credentials, rotating panel secrets."
        postgres_pw="$(grep -oE 'POSTGRES_PASSWORD: \$\{POSTGRES_PASSWORD:-[^}]*\}' docker-compose.yml | head -1 | sed 's/.*:-//; s/}.*//')"
        [ -z "$postgres_pw" ] && postgres_pw="$(grep -oE 'POSTGRES_PASSWORD: [^ $]+' docker-compose.yml | head -1 | awk '{print $2}')"
        [ -z "$postgres_pw" ] && postgres_pw="panel_password_secure_99"
        admin_pw="admin12345"
    else
        info "Fresh install detected — generating strong random secrets."
    ...
    umask 077
    cat > "$env_file" <<EOF
# Wings Panel secrets — generated by install.sh. KEEP THIS FILE SECRET. Never commit it.
...
EOF
    ...
    umask 022
    set -a; . "$env_file"; set +a
    ok "Secrets generated in $env_file (chmod 600). Keep a backup!"
}
...
summary() {
    ...
    local admin_user=""
    local admin_pass=""
    local admin_login="${ADMIN_PASSWORD:-admin12345}"
    if curl -sf http://localhost:$panel_port/api/auth/login -X POST \
        -H "Content-Type: application/json" \
        -d "{\"username\":\"admin\",\"password\":\"$admin_login\"}" >/dev/null 2>&1; then
        admin_user="admin"
    ...
    if [ -n "$admin_user" ]; then
        ...
        if [ "$admin_pass" = "admin12345" ]; then
            echo -e "  ${RED}WARNING:${NC} the default password admin12345 is still in use — change it soon!"
        fi
    else
